...
<template>
    <!-- 顶部标题 -->
    <view class="nav" v-if="type === 'page'">
        <text>chat</text>
        <view class="text" @tap="newChat">新对话</view>
    </view>
    <!-- 聊天记录 -->
    <scroll-view class="scroll-container" scroll-y @scroll="scrollViewScroll" :scroll-top="scrollTop" :class="{modal:type==='modal'}">
    ...
    <!-- 底部问题输入框 -->
    <view class="footer">
        <chat-input v-model:value="inputVal" @send="send" @stop="stopAnswer"></chat-input>
    </view>
</template>
...
import { stopChat, getDetail } from '../../pages/chat/api'
